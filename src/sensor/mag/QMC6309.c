...
int qmc_update_odr(float time, float *actual_time)
{
	...
	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_MAG, QMC6309_CTRL_REG_1, LPF_MASK(LPF_OFF) | OSR_MASK(OSR_OFF) | MD);
	if (err)
		LOG_ERR("Communication error");
...
{
	int err = ssi_reg_write_byte(SENSOR_INTERFACE_DEV_MAG, QMC6309_CTRL_REG_1, LPF_MASK(LPF_OFF) | OSR_MASK(OSR_OFF) | MD_SINGLE);
